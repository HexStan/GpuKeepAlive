...
- 版本号的唯一来源是仓库根目录的 `VERSION` 文件（格式 `x.y.z`，语义化版本）；csproj 不单独设置版本号，无需改动
- 发布操作须在默认分支 `master` 的最新代码上进行，除 `VERSION` 外工作区无未提交改动
- 撰写发布注记前，先查看既有 Release 的行文风格：`gh release view vX.Y.Z`

## 发布步骤

> 以下 `x.y.z` 指本次发布的版本号，即 `VERSION` 文件中待提交的现有内容。

### 1. 确认版本号就绪

- 读取根目录 `VERSION` 的现有内容，作为本次发布的版本号 `x.y.z`（Agent 不决定进位、不修改该文件）
- 确认 `VERSION` 存在未提交的修改（`git show HEAD:VERSION` 与 `VERSION` 不一致；脚本亦会校验，若无未提交修改将中止发布）
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/release.ps1 -NotesFile dist/release-notes-vx.y.z.md
```

脚本将依次执行：

1. **校验**：从 `VERSION` 读取版本号并校验格式、确认 `VERSION` 有未提交修改、除 `VERSION` 外工作区干净、`gh` 登录状态、本地不落后远端、tag/Release 不重复
...
4. **提交版本号并打 tag**：确认后提交 `chore: 版本号升至 x.y.z`，创建 `vx.y.z` 指向该提交，推送分支与 tag 到 origin
...
| `-AllowDirty` | 允许除 `VERSION` 外存在未提交改动 |
| `-Yes` | 跳过发布前确认 |
...
# 提交版本号（维护者已修改 VERSION 但未提交），打 tag 并推送
git commit -m "chore: 版本号升至 x.y.z" -- VERSION
git tag vx.y.z
